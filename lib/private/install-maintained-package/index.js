...
const setupPackageRepostory = async (
	{ path, meta: { multiPackageRepoName, repoUrl } },
	{ multiPackageReposMeta },
	inputOptions
) => {
	if (!multiPackageRepoName) return setupRepository(path, repoUrl, inputOptions);
	const multiPackageRepoMeta = multiPackageReposMeta[multiPackageRepoName];
	return setupRepository(multiPackageRepoMeta.path, multiPackageRepoMeta.repoUrl, inputOptions);
};
