...
const setupPackageRepostory = async ({ path, meta }, { multiPackageReposMeta }, inputOptions) => {
	if (!meta.multiPackageRepoName) return setupRepository(path, meta, inputOptions);
	const multiPackageRepoMeta = multiPackageReposMeta[meta.multiPackageRepoName];
	return setupRepository(multiPackageRepoMeta.path, multiPackageRepoMeta, inputOptions);
};
...
module.exports = async (packageContext, userConfiguration, inputOptions, progressData) => {
	...
	packageContext.installationJobs = await setupPackageRepostory(
		packageContext, userConfiguration, inputOptions
	);
