...
const generateLink = async dependencyContext => {
	const { dependentContext, name, path, isSemVerVersionRange } = dependencyContext;
	...
	if (isSemVerVersionRange) validateVersion(dependencyContext);
};
...
module.exports = async (dependencyContext, userConfiguration, progressData) => {
	...
	if (await isSymlink(path, { linkPath: linkedPath })) {
		validateVersion(dependencyContext);
		return;
	}
