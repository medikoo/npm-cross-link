...
module.exports = memoizee(
	async (name, version, externalContext) => {
		...
		const versionDirName = Buffer.from(version).toString("base64");
		const versionCachePath = resolve(cachePath, name, versionDirName);
		if (await isDirectory(versionCachePath)) return versionCachePath;
		...
		copyDir(targetDirname, versionCachePath);
