...
module.exports = memoizee(
	async (name, version, externalContext) => {
		...
		const packageTmpDir = resolve(tmpdir, "npm-cross-link", name, versionDirName);
		...
		if (versionMetaData) {
			...
			await new Promise((promiseResolve, promiseReject) => {
				const stream = got
					...
					.pipe(tar.x({ cwd: packageTmpDir, strip: 1 }));
			...
			const sourcePath = resolve(packageTmpDir, "node_modules", name);
			await Promise.all(
				(await readdir(sourcePath)).map(filename => {
					if (filename === "." || filename === "..") return null;
					return rename(resolve(sourcePath, filename), resolve(packageTmpDir, filename));
				})
			);
