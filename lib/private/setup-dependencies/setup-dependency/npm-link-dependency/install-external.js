...
module.exports = async dependencyContext => {
	...
	log.notice("%s updating %s to %s", dependent.name, dependency.name, latestSupportedVersion);
	...
	const sourceDirname = await muteErrorIfOptional(dependencyContext, () =>
		prepareDependency(dependency.name, latestSupportedVersion, metadata)
	);
	if (!sourceDirname) return;
	await copyDir(sourceDirname, dependency.path);

	// Ensure to map binaries
	await Promise.all(
		Object.entries(getPackageJson(dependency.path).bin || {}).map(
			async ([targetName, linkedPath]) => {
				...
				await symlink(join("../", dependency.name, linkedPath), targetPath, {
					intermediate: true
				});
			}
		)
