...
module.exports = async (dependencyContext, progressData) => {
	const { dependent, dependencyName, isExternal } = dependencyContext;
	const dependencyPath = (dependencyContext.dependencyPath = resolve(
		dependent.path, "node_modules", dependencyName
	));
	...
	log.info("%s link dependency %s", dependent.name, dependencyName);
