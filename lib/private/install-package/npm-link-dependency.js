...
const resolveExternalDependencyUpgradeVersion = async dependencyContext => {
	const { dependentName, dependentPath, dependencyName, linkedPath } = dependencyContext;
...
module.exports = async dependencyContext => {
	...
	const linkVersion =
		isExternal && (await resolveExternalDependencyUpgradeVersion(dependencyContext));
	if (!linkVersion && (await isSymlink(dependencyLinkPath, { linkPath: linkedPath }))) return;
