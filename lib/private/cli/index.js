...
const log                      = require("log").get("npm-cross-link")
    ...
    , installPackageGlobally   = require("../../../install-package-globally")
    , installDependency        = require("../../../install-dependency")
	...
	const installPromise = (() => {
		...
		if (packageName) {
			if (inputOptions.global) {
				const isExternal = !userConfiguration.packagesMeta[packageName];
				return isExternal
					? installPackageGlobally(packageName, userConfiguration, inputOptions)
					: installMaintainedPackage(packageName, userConfiguration, inputOptions);
			}
			return installDependency(process.cwd(), packageName, userConfiguration, inputOptions);
		}
