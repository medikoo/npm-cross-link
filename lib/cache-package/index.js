...
module.exports = memoizee(
	async (name, version, externalContext) => {
		...
		if (versionCachePath) {
			const versionCachePathStats = await lstat(versionCachePath, { loose: true });
			if (versionCachePathStats) {
				if (versionCachePathStats.isDirectory()) return versionCachePath;
				await unlink(versionCachePath);
			}
		}
