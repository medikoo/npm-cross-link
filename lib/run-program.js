...
module.exports = (command, args, options = {}) => {
	...
	return promise.catch(error => {
		if (!logger && error.stdBuffer) {
			log.warn(
				"%s failed for %s, %O with %#s, output:\n%s", command, args.join(" "), options,
				error.message, error.stdBuffer
			);
		} else {
			log.warn(
				"%s failed for %s, %O with %#s", command, args.join(" "), options, error.message
			);
		}
		throw error;
	});
